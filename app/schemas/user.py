...
class UserUpdate(BaseModel):
    ...
    full_name: Optional[str] = Field(None, min_length=1, max_length=100, description="실명")
    phone: Optional[str] = Field(None, max_length=20, description="전화번호")
...
class NotificationSettings(BaseModel):
    ...
    notification_advance_hours: int = Field(..., ge=1, le=24, description="알림 시간 (시간 단위, 1-24)")


class NotificationSettingsUpdate(BaseModel):
    """알림 설정 수정 스키마"""
    push_notification_enabled: Optional[bool] = Field(None, description="푸시 알림 활성화")
    notification_advance_hours: Optional[int] = Field(None, ge=1, le=24, description="알림 시간 (시간 단위, 1-24)")
