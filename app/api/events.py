...
@router.get("/", response_model=List[EventResponse], summary="이벤트 목록 조회", description="사용자의 모든 경조사 이벤트를 조회합니다.")
def get_events(
...
@router.get("/{event_id}", response_model=EventResponse, summary="이벤트 상세 조회", description="특정 이벤트의 상세 정보를 조회합니다.")
def get_event(
    event_id: int,
    current_user_id: int = Depends(get_current_user_id),
    db: Session = Depends(get_db)
...
@router.put("/{event_id}", response_model=EventResponse, summary="이벤트 수정", description="기존 이벤트를 수정합니다.")
def update_event(
    event_id: int,
...
@router.get("/upcoming", response_model=List[EventResponse], summary="다가오는 이벤트 조회", description="다가오는 이벤트들을 조회합니다.")
...
@router.get("/type/{event_type}", response_model=List[EventResponse], summary="타입별 이벤트 조회", description="특정 타입의 이벤트들을 조회합니다.")
...
@router.get("/calendar", response_model=List[CalendarEventResponse], summary="캘린더 이벤트 조회", description="캘린더에 표시할 이벤트들을 조회합니다.")
...
@router.post("/calendar", response_model=CalendarEventResponse, summary="캘린더 이벤트 생성", description="캘린더에 표시할 새로운 이벤트를 생성합니다.")
...
@router.get("/stats", summary="이벤트 통계", description="사용자의 이벤트 통계를 조회합니다.")
def get_event_statistics(
    ...
    # 이벤트 타입별 통계
    event_type_stats = {}
    event_types = db.query(Event.event_type).filter(Event.user_id == current_user_id).distinct().all()
    ...
    # 외부 이벤트 수
    external_events = db.query(Event).filter(
        Event.user_id == current_user_id,
