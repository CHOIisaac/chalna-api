...
@router.get("/", response_model=List[LedgerResponse], summary="장부 목록 조회", description="사용자의 모든 장부 기록을 조회합니다.")
def get_ledgers(
...
@router.get("/{ledger_id}", response_model=LedgerResponse, summary="장부 상세 조회", description="특정 장부 기록의 상세 정보를 조회합니다.")
def get_ledger(
    ledger_id: int,
    current_user_id: int = Depends(get_current_user_id),
    db: Session = Depends(get_db)
...
@router.put("/{ledger_id}", response_model=LedgerResponse, summary="장부 기록 수정", description="기존 장부 기록을 수정합니다.")
def update_ledger(
    ledger_id: int,
    ...
    """장부 기록 수정"""
    db_ledger = db.query(Ledger).filter(
        Ledger.id == ledger_id,
        Ledger.user_id == current_user_id
    ).first()
...
@router.delete("/{ledger_id}", summary="장부 기록 삭제", description="장부 기록을 삭제합니다.")
def delete_ledger(
    ledger_id: int,
    current_user_id: int = Depends(get_current_user_id),
    db: Session = Depends(get_db)
...
@router.get("/income", response_model=List[LedgerResponse], summary="수입 내역 조회", description="수입 내역만 조회합니다.")
...
@router.get("/expense", response_model=List[LedgerResponse], summary="지출 내역 조회", description="지출 내역만 조회합니다.")
...
@router.get("/balance", summary="수지 잔액 조회", description="총 수입, 지출, 잔액을 조회합니다.")
def get_balance(
    current_user_id: int = Depends(get_current_user_id),
    db: Session = Depends(get_db)
):
...
@router.post("/search", response_model=List[LedgerResponse], summary="장부 검색", description="조건에 맞는 장부 기록을 검색합니다.")
def search_ledgers(
    search: LedgerSearch,
    skip: int = Query(0, ge=0, description="건너뛸 항목 수"),
    limit: int = Query(100, ge=1, le=1000, description="가져올 항목 수"),
...
@router.get("/stats", response_model=LedgerStatistics, summary="장부 통계", description="사용자의 장부 통계를 조회합니다.")
...
@router.post("/quick-add", response_model=LedgerResponse, summary="빠른 장부 추가", description="간단한 정보로 장부 기록을 빠르게 추가합니다.")
...
@router.get("/event-type/{event_type}", response_model=List[LedgerResponse], summary="경조사 타입별 조회", description="특정 경조사 타입의 장부 기록을 조회합니다.")
...
@router.get("/relationships", summary="관계별 통계", description="관계별 장부 통계를 조회합니다.")
def get_relationship_statistics(
    ...
    # 관계 타입별 통계
    relationship_stats = {}
    relationships = db.query(Ledger.relationship_type).filter(
        Ledger.user_id == current_user_id
    ).distinct().all()
