...
def update_current_user(
    ...
    if "username" in update_data:
        existing_user = (
            db.query(User)
            .filter(
                User.username == update_data["username"], User.id != current_user_id
            )
            .first()
        )
        if existing_user:
    ...
    if "email" in update_data:
        existing_user = (
            db.query(User)
            .filter(User.email == update_data["email"], User.id != current_user_id)
            .first()
        )
        if existing_user:
            raise HTTPException(status_code=400, detail="이미 사용 중인 이메일입니다")
...
@router.patch(
    ...
    response_model=NotificationSettings,
    summary="알림 설정 변경",
    description="현재 사용자의 알림 설정을 변경합니다.",
)
...
@router.get(
    "/me/stats", summary="내 통계", description="현재 사용자의 통계 정보를 조회합니다."
)
